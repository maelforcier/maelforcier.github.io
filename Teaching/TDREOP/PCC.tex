...
\pause
Preuve: Réductions \\
Problème du plus court chemin élementaire.\\
\pause
\medskip
...
\subsection{Programmation dynamique : cas général}


\begin{frame}\frametitle{Programmation dynamique}
$V(t,x):=$ coût minimal futur lorsqu'on est dans l'état $x$ à l'étape $t$.
...
$d_t$ : demande pour la période $t$ (supposée connue) 

$x_t\in\mathbb{Z}$ : nombre d'unités disponibles en début de période $t$ 

$u_t$ : nombre d'unités commandées (et reçues immédiatement) en début de période $t$ 

$K$ : capacité maximale de stockage

$c(u_t)+g(x_{t+1})$ : coût de gestion de stock pour la période $t$ en
\begin{itemize}
\item $c(u_t)$ coût de réapprovisionnement
\item  $g(x_{t+1})$ coût de stockage ou coût de pénurie 
\end{itemize}
...
{\red périodes} : périodes $t=1,\ldots, T$

\bigskip

{\red états} : valeurs possibles $x_t$ de niveau de stock, 

\bigskip

{\red transitions} : $x_t\rightarrow x_{t+1}$ tq $x_{t+1}$ satisfasse simultanément $x_{t+1}\in\mathbb{Z}$, $x_{t+1}+d_t\leq K$ et $x_{t+1}\geq x_t-d_t$ 

\bigskip


{\red coût de la transition} : $x_t\rightarrow x_{t+1}$ est $c(x_{t+1}-x_t+d_t)+g(x_{t+1})$

\bigskip
\bigskip

{\red Objectif.} Minimiser $\sum_{t=0}^{N-1}c(x_{t+1}-x_t+d_t)+g(x_{t+1})$  (critère additif = somme des coûts des transitions de la trajectoire).
