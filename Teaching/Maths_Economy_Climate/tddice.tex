...
\begin{document}
	...
	\noindent
	{\textbf{Universit\'e de Nantes} \hfill Ann\'ee 2025-2026}\\
...
L'équation de Cobb-Douglas, classique en macroéconomie pour étudier la croissance, fait l'hypothèse que le PIB $Q_n$ est égal au produit $AK_n^\gamma L_n^{1-\gamma}$ où $A$ est appelé le facteur de productivité, $\gamma$ l'élasticité du capital et $L_n$ le travail, souvent approximé comme étant égal à la population.
...
Enfin, Nordhaus estime que le coût de l'investissement $\alpha_n$ dans les technologies bas carbone s'exprime comme $0,0686 \mu_n^{2,887}$. Pour simplifier, on pose $\alpha_n = 0,05\mu_n^3$.
\ques{Donner une expression simplifiée de  $\Omega_n$ en fonction de $\mu_n$ et $T_n$.}
\rep{
\begin{equation*}
...
\rep{
\begin{equation*}
\forall n, \quad \Omega_n = \frac{1-0,05}{1+0,0133 (\frac{T_0}{3})}=\frac{0,95}{1,000945778}=0,9491
\end{equation*}
\begin{equation*}
\forall n, \quad  K_n  =\frac{1- \delta}{1- \frac{0,9491}{4} } K_{n-1} =\frac{1- \delta}{1- 0,2373 } K_{n-1} = \frac{1- \delta}{0,7627} K_{n-1}
\end{equation*}
}

\ques{Comment peut-on qualifier mathématiquement la suite $(K_n)_{n\in \NN}$ ? \label{ques:suite_geo}}
\rep{$(K_n)_{n\in \NN}$ est une suite géométrique de raison $\frac{1- \delta}{0,7627}$.}

\ques{Calculer $K_n$ pour tout $n$, en fonction de $K_0$ et $\delta$.}
\rep{\begin{equation*}
\forall n, \quad  K_n = K_0 \left(\frac{1- \delta}{0,7627}\right)^n 
\end{equation*}}
...
Dans cette partie, on propose une ``heuristique" pour le planificateur, c'est-à-dire une stratégie simple qui fonctionne mieux que des stratégies naïves mais sans être optimale pour autant. On suppose que l'heuristique uilisée par le planificateur est la suivante. Tant que la croissance du capital $r_n = K_n / K_{n-1}$ est supérieure une constante $r_\star$, on ne fait rien$\,$; en revanche, dès que $r_n \leqslant r_\star$, on contrôle totalement. Autrement dit, on suppose que
$$
 \mu_n = 0 \quad \text{si} \quad r_n > r_\star \quad \text{et} \quad \mu_n = 1 \quad \text{sinon.}
$$

\ques{Proposer une valeur de $r_\star$ cohérente pour maximiser la croissance du capital $K^n$.}
\rep{
On propose de prendre $r_\star$ comme la raison de la suite géométrique définissant la croissance du capital dans le cas d'un contrôle total :
$$
r_\star := \frac{K_n}{K_{n-1}} = \frac{1- \delta}{0,7627}.
$$
}
...
\begin{equation*}
\begin{aligned}
&(1-\delta)\frac{4}{3}\left(1-\frac{d_n}{3}\right) < \frac{1-\delta}{0,7627} \\
